...
class Driver(object):
  ...
  def write_telemetry(self, stream, telemetry_datum, binary=False, **extra_headers):
    ...
    active. The pipelines will then pass the telemetry datum along to their sessions, which will in turn send it to 
    ...
    @note Occasionally, a pipeline's telemetry stream may be throttled to relieve excess network load. Because telemetry
          data is tied to a timestamp, any telemetry data the pipeline receives when it is being throttled will be
